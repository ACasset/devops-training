...
Cet inventaire comporte quelques trous. Vous savez en principe déjà comment compléter les variables `ansible_user` à la ligne 8, et `ansible_host` aux lignes 17 et 25.

Les variables `nom_utilisateur` aux lignes 18 et 26 sont pré-remplies, mais vous pouvez les modifier à votre convenance. Gardez juste à l'esprit que l'on va créer des utilisateurs Unix avec, donc évitez les caractères spéciaux.

Enfin, les variables `mot_de_passe_utilisateur` aux lignes 19 et 27 sont un peu plus délicates : il nous faut absolument éviter de mettre en clair des éléments sensibles tels qu'un mot de passe dans un fichier YAML, car celui-ci va être typiquement versionné et publiquement accessible sur un dépôt de code.
